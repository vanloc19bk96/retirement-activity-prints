...
} from '@/types/studio-crossword.types'
...
async function parseError(response: Response): Promise<Error> {
  try {
    ...
    return new Error(`Crossword clue generation failed (${response.status})`)
  } catch {
    return new Error(`Crossword clue generation failed (${response.status})`)
  }
}
...
export async function generateCrosswordClues(
  ...
  const response = await fetch('/api/studio/crossword-clues', {
    method: 'POST',
    signal,
  ...
  return response.json() as Promise<CrosswordCluesResponse>
}

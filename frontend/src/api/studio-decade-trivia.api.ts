...
async function parseError(response: Response): Promise<Error> {
  try {
    ...
    return new Error(`Decade trivia generation failed (${response.status})`)
  } catch {
    return new Error(`Decade trivia generation failed (${response.status})`)
  }
}
...
export async function generateTrivia(
  ...
  const response = await fetch('/api/studio/decade-trivia', {
    method: 'POST',
    signal,
  ...
  return response.json() as Promise<DecadeTriviaResponse>
}

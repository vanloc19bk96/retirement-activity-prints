...
async function parseError(response: Response): Promise<Error> {
  try {
    ...
    return new Error(`Anagram generation failed (${response.status})`)
  } catch {
    return new Error(`Anagram generation failed (${response.status})`)
  }
}

export async function generateAnagram(
  req: AnagramRequest,
  signal: AbortSignal,
): Promise<AnagramResponse> {
  const response = await fetch('/api/studio/anagram', {
    method: 'POST',
    signal,
  ...
  return response.json() as Promise<AnagramResponse>
}

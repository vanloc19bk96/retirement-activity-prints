import type { StoryRecallRequest, StoryRecallResponse } from '@/types/studio-story.types'
...
async function parseError(response: Response): Promise<Error> {
  try {
    ...
    return new Error(`Story generation failed (${response.status})`)
  } catch {
    return new Error(`Story generation failed (${response.status})`)
  }
}
...
export async function generateStory(
  ...
  return response.json() as Promise<StoryRecallResponse>
}

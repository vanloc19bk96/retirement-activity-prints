import type { PairRequest, PairResponse } from '@/types/studio-pairs.types'
...
async function parseError(response: Response): Promise<Error> {
  try {
    ...
    return new Error(`Pair generation failed (${response.status})`)
  } catch {
    return new Error(`Pair generation failed (${response.status})`)
  }
}
...
export async function generatePairs(
  ...
  const response = await fetch('/api/studio/pairs', {
    method: 'POST',
    signal,

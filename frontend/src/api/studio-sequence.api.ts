import type { SequenceRequest, SequenceResponse } from '@/types/studio-sequence.types'
...
async function parseError(response: Response): Promise<Error> {
  try {
    ...
    return new Error(`Sequence generation failed (${response.status})`)
  } catch {
    return new Error(`Sequence generation failed (${response.status})`)
  }
}
...
export async function generateSequences(
  ...
  const response = await fetch('/api/studio/sequence', {
    method: 'POST',
    signal,
  ...
  return response.json() as Promise<SequenceResponse>
}

...
async function parseError(response: Response): Promise<Error> {
  try {
    ...
    return new Error(`Category fluency generation failed (${response.status})`)
  } catch {
    return new Error(`Category fluency generation failed (${response.status})`)
  }
}
...
export async function generateCategory(
  ...
  const response = await fetch('/api/studio/category-fluency', {
    method: 'POST',
    signal,
  ...
  return response.json() as Promise<CategoryFluencyResponse>
}
